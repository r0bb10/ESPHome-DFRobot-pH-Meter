...
void DFRobotPHMeter::evaluate_calibration_mode_() {
  ...
  bool has_ph4 = acid_voltage_pref_.load(&ph4_v) && ph4_v > MIN_CALIBRATION_VOLTAGE;
  bool has_ph7 = neutral_voltage_pref_.load(&ph7_v) && ph7_v > MIN_CALIBRATION_VOLTAGE;
  bool has_ph10 = alkaline_voltage_pref_.load(&ph10_v) && ph10_v > MIN_CALIBRATION_VOLTAGE;
  ...
  if (count >= 3) {
    ...
    if (has_ph4 && has_ph7) set_calibration_pair(4, 7);
    else if (has_ph7 && has_ph10) set_calibration_pair(7, 10);
    else set_calibration_pair(4, 10);
    ESP_LOGI(TAG, "Detected 2-point calibration (pH%d, pH%d)", cal_point_1_, cal_point_2_);
  } else {
    use_three_point_ = true;
    ESP_LOGI(TAG, "Only one calibration point: fallback to default 3-point mode");
  }
}
