...
CONFIG_SCHEMA = cv.Schema({
    ...
    cv.Optional("use_ads1115", default=True): cv.boolean,
    ...
    cv.Optional(CONF_ALKALINE_VOLTAGE): cv.positive_float,
    ...
    cv.Optional(CONF_CALIBRATION_MODE_TYPE, default="2_point"): cv.one_of("2_point", "3_point", lower=True),
    cv.Optional(CONF_CALIBRATION_POINTS, default=["pH4", "pH7"]): cv.ensure_list(cv.one_of("pH4", "pH7", "pH10")),
...
async def to_code(config):
    ...
    if CONF_ALKALINE_VOLTAGE in config:
        cg.add(var.set_acid_voltage(config[CONF_ACID_VOLTAGE]))
        cg.add(var.set_neutral_voltage(config[CONF_NEUTRAL_VOLTAGE]))
        cg.add(var.set_update_interval(config[CONF_UPDATE_INTERVAL]))
    ...
    if config.get(CONF_CALIBRATION_MODE_TYPE) == "3_point":
        cg.add(var.set_use_three_point(True))

    if config[CONF_TEMPERATURE_UNIT] == "fahrenheit":
        cg.add(var.set_use_fahrenheit(True))

    points = config[CONF_CALIBRATION_POINTS]
    p1 = CALIBRATION_LABELS[points[0]]
    p2 = CALIBRATION_LABELS[points[1]]
    cg.add(var.set_calibration_pair(p1, p2))
